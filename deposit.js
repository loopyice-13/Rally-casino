...
function validateScreenshot(file) {
  ...
  if (file.size > MAX_FILE_SIZE) return `Screenshot must be under ${APP.maxUploadMB} MB. Your file is ${humanMB(file.size)}.`;
  return null;
}
...
async function handleDepositSubmit(e) {
  ...
  if (!captchaToken) return toast('Please complete the captcha.', true);
